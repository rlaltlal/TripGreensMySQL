...
<script>
	...
	$(document).ready(function() {
		$("#btnWrite").click(function() {
			location.href = "${path}/Board/WriteForm2";
		});
	});

	function list(page) {
		location.href="${path}/Board/List2?curPage="+page+"&searchOption=${map.searchOption}"+"&keyword=${map.keyword}";
	}
</script>
<style>
...
<h2 class="head2">1:1 문의 게시판</h2>
  ...
  <c:forEach  var="board" items="${map.list2}">
    <tr>
      <td>
      	${ board.qseq }
      </td>
      <td>
        ...
        <c:choose>
        <c:when test="${sessionScope.email == 'www.admin@admin.com' || sessionScope.email == board.email }">
        <a href="${path}/Board/View2?qseq=${board.qseq}&email=${board.email}">${board.title}</a>
        </c:when>
        <c:otherwise>
        <a>${board.title}</a>
        </c:otherwise>
  ...
  <br>
  <div id="board2">
  <form name="form1" method="post" action="${path}/Board/List2">
  	...
  	<button type="button" id="btnWrite">1:1 문의하기</button>
  	</c:if>
  </form>
  	</div>
 </div> 
