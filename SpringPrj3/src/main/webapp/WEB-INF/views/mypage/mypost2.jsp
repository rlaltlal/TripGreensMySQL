...
<body>
  ...
  <br><br>
<div id="board">
  <form name="form1" method="post" action="/Mypage/List?email=${vo.email}">
					...
					<ul class="pagination">
						<c:if test="${pageMaker.prev}">
							<li class="paginate_button previous"><a
								href="/Mypage/List?email=${vo.email}&pageNum=${pageMaker.startPage -1}">Previous</a></li>
						</c:if>
						<c:forEach var="num" begin="${pageMaker.startPage}"
							end="${pageMaker.endPage}">
							<li class="paginate_button  ${pageMaker.cri.pageNum == num ? "active":""} ">
								<a href="/Mypage/List?email=${vo.email}&pageNum=${num}">${num}</a>
							</li>
						</c:forEach>

						<c:if test="${pageMaker.next}">
							<li class="paginate_button next"><a
								href="/Mypage/List?email=${vo.email}&pageNum=${pageMaker.endPage +1 }">Next</a></li>
						</c:if>
